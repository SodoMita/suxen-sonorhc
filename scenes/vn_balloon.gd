...
## absent: text still types out, but no per-character blip is played or offered
## in Settings.
##
...
func _repaint_current_line() -> void:
	...
	_apply_speaker_chrome(dialogue_line.character)
	_refresh_stats()
...
func apply_dialogue_line() -> void:
	...
	if dialogue_line.responses.size() > 0:
		...
		if (_skip_key_held or _resume_skip_after_choice or Input.is_action_pressed(skip_action)) and not _any_overlay_open():
			...
			skip_button.modulate = MODE_TINT
...
const SPEAKER_COLORS := {
	"Ren": Color(0.62, 0.86, 1.0),
	"Aurora": Color(0.55, 0.86, 1.0),
	"Kira": Color(1.0, 0.64, 0.38),
	"Elara": Color(0.52, 0.9, 0.68),
	"Selene": Color(0.8, 0.6, 1.0),
	"Aria": Color(0.96, 0.9, 0.74),
	"Nova": Color(0.62, 0.76, 1.0),
}
const ACCENT := Color(0.72, 0.86, 1.0)
const MODE_TINT := Color(0.62, 0.88, 1.0)

var _bg_tween: Tween


func _apply_speaker_chrome(who: String) -> void:
	var tint: Color = SPEAKER_COLORS.get(who, ACCENT)
	character_label.add_theme_color_override("default_color", tint)
	next_indicator.color = tint
	var base: StyleBox = name_plate.get_theme_stylebox("panel")
	if base is StyleBoxFlat:
		var styled := (base as StyleBoxFlat).duplicate() as StyleBoxFlat
		styled.border_color = Color(tint.r, tint.g, tint.b, 0.82)
		name_plate.add_theme_stylebox_override("panel", styled)


func _bind_story_stats() -> void:
	var gs := get_tree().root.get_node_or_null("GameState")
	if gs == null:
		return
	if gs.has_signal("stat_changed") and not gs.stat_changed.is_connected(_on_story_stat_changed):
		gs.stat_changed.connect(_on_story_stat_changed)
	if gs.has_signal("bond_changed") and not gs.bond_changed.is_connected(_on_story_bond_changed):
		gs.bond_changed.connect(_on_story_bond_changed)


func _on_story_stat_changed(_stat_name: String, _value: int) -> void:
	_refresh_stats()


func _on_story_bond_changed(_character_id: String, _value: int) -> void:
	_refresh_stats()


func _refresh_stats() -> void:
	if not is_instance_valid(stat_strip):
		return
	var gs := get_tree().root.get_node_or_null("GameState")
	if gs == null:
		stat_strip.text = ""
		return
	var text := "Trust %d    Insight %d    Power %d" % [int(gs.trust), int(gs.insight), int(gs.power)]
	if is_instance_valid(dialogue_line) and not dialogue_line.character.is_empty() and gs.has_method("get_bond"):
		var bond := int(gs.get_bond(dialogue_line.character))
		if bond > 0:
			text += "    ·    %s %d" % [dialogue_line.character, bond]
	stat_strip.text = text
...
func _set_background(key: String) -> void:
	...
	if background.texture == next_tex and key != "none":
		return
	background.texture = next_tex
	if next_tex == null:
		background.modulate.a = 1.0
		return
	background.modulate.a = 0.15
	if _bg_tween != null and _bg_tween.is_valid():
		_bg_tween.kill()
	_bg_tween = background.create_tween()
	_bg_tween.tween_property(background, "modulate:a", 1.0, 0.32)
...
func _toggle_auto() -> void:
	...
	auto_button.modulate = MODE_TINT if auto_mode else Color.WHITE
